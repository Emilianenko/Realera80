                                   ...
                                   Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/)
{
	...
	if (item->isStackable()) {
		...
		if (item->equals(toItem) && !item->isRune()) {
                                  ...
                                  uint32_t flags, bool test, uint32_t& remainderCount)
{
	...
	*/
	...
	if (item->isStackable() && item->equals(toItem) && !item->isRune()) {
...
void Game::playerSpeakToNpc(Player* player, const std::string& text)
{
	...
	for (Creature* spectator : list) {
		if (spectator->getNpc()) {
			spectator->onCreatureSay(player, TALKTYPE_CHANNEL_O, text);
		}
	}
}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			sss << realHealthChange;
			addAnimatedText(list, targetPos, TEXTCOLOR_MAYABLUE, sss.str());
		}
	} else {
		SpectatorVec list;
		map.getSpectators(list, targetPos, true, true);
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				std::stringstream sss;
				sss << manaDamage;
				addAnimatedText(list, targetPos, TEXTCOLOR_BLUE, sss.str());
		...
		std::stringstream sss;
		sss << realDamage;
		addAnimatedText(list, targetPos, message.primary.color, sss.str());
...
void Game::sendGuildParticipants(uint32_t playerId, std::string chmembers)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	...
	if (guild) {
		if (chmembers.empty()) {
			player->sendChannelMessage("Guild channel participants", player->getName(), TALKTYPE_CHANNEL_O, CHANNEL_GUILD);
			return;
		}

		player->sendChannelMessage("Guild channel participants", chmembers, TALKTYPE_CHANNEL_O, CHANNEL_GUILD);
	}
}
